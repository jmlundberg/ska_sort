...
{
    std::ptrdiff_t num_elements = end - begin;
    if (num_elements < 128 || detail::radix_sort_pass_count<std::invoke_result_t<ExtractKey,decltype(*begin)>> >= 8)
    {
        ska::sort(begin, end, key);
        return false;
    }
    else
        return detail::RadixSorter<std::invoke_result_t<ExtractKey,decltype(*begin)>>::sort(begin, end, buffer_begin, key);
}
